...
const App = {
  ...
  retryTest(mode) {
    ...
    if (this.isReviewTest) {
      this.startReviewTest(this._reviewTestMode || mode);
      return;
    }
  ...
  startReviewTest(mode) {
    this.isReviewTest = true;
    this._reviewTestMode = mode; // 復習テストのモードを記録
    // 現在の学年で実際に「弱点」ステータスの漢字を取得
    const kanjiList = getKanjiByGrade(this.currentGrade);
    let reviewItems = [];

    kanjiList.forEach(k => {
      const status = Storage.getKanjiStatus(k.char, mode);
      if (status.color === 'red' && status.reviewStreaks) {
        Object.entries(status.reviewStreaks).forEach(([reading, info]) => {
          if (info.hasError && info.streak < status.masteryStreak) {
    ...
    if (reviewItems.length === 0) {
      alert(`${this.currentGrade}年生で${mode === 'reading' ? '読み' : '書き'}の弱点漢字はありません！\n素晴らしいです！`);
      this.isReviewTest = false;
      return;
    }

    const shuffled = reviewItems.sort(() => 0.5 - Math.random());
